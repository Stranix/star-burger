...
### Для Docker предусмотрен deploy скрипт или скрипт обновления
Чтобы правильно утелело уведомление в Rollbar надо не забыть задать переменную среды `ROLLBAR_POST_ACCESS_TOKEN`  
...
Скрипт выполнит:
 ...
 - Затянет измениня с github
 - Пересоберет докер образы и запустит контейнеры
 - Выполнит сбор статики Django
 - Отпрвит уведомление о деплое в Rollbar если верной задан токен доступа  

Миграции, в целях избежания лишних ошибок, накатываются в ручную. Команду можно найти выше.
